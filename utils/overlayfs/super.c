...
static int ovl_show_options(struct seq_file *m, struct dentry *dentry)
{
	...
	if (ofs->config.has_override_creds)
		seq_printf(m, ",override_creds=%u:%u",
			   from_kuid_munged(&init_user_ns,
					    ofs->config.override_uid),
			   from_kgid_munged(&init_user_ns,
					    ofs->config.override_gid));
	return 0;
}
...
static const match_table_t ovl_tokens = {
	...
	{OPT_OVERRIDE_CREDS,		"override_creds=%s"},
	{OPT_ERR,			NULL}
};
...
static int ovl_parse_opt(char *opt, struct ovl_config *config)
{
	...
	while ((p = ovl_next_opt(&opt)) != NULL) {
		...
		{
			char *cred_str, *gid_ptr;
			unsigned int uid_val, gid_val;

			cred_str = match_strdup(&args[0]);
			if (!cred_str)
				return -ENOMEM;

			/* Format is uid:gid */
			gid_ptr = strchr(cred_str, ':');
			if (!gid_ptr) {
				pr_err("bad mount option \"override_creds=%s\", expected uid:gid\n",
				       cred_str);
				kfree(cred_str);
				return -EINVAL;
			}
			*gid_ptr = '\0';
			...
			if (kstrtouint(cred_str, 10, &uid_val)) {
				pr_err("bad uid in \"override_creds\"\n");
				kfree(cred_str);
				return -EINVAL;
			}
			if (kstrtouint(gid_ptr, 10, &gid_val)) {
				pr_err("bad gid in \"override_creds\"\n");
				kfree(cred_str);
				return -EINVAL;
			}
			kfree(cred_str);

			config->override_uid = make_kuid(current_user_ns(), uid_val);
			if (!uid_valid(config->override_uid)) {
				pr_err("invalid uid %u in \"override_creds\"\n",
				       uid_val);
				return -EINVAL;
			}
			config->override_gid = make_kgid(current_user_ns(), gid_val);
			if (!gid_valid(config->override_gid)) {
				pr_err("invalid gid %u in \"override_creds\"\n",
				       gid_val);
				return -EINVAL;
			}
			config->has_override_creds = true;
			break;
		}
			    ...
			    struct path *workpath)
{
	...
	rename_whiteout = err;
	ofs->rename_whiteout = rename_whiteout;
	if (!rename_whiteout)
...
static int ovl_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	/*
	 * Operations wrapped in ovl_override_creds() run with creator_cred.
	 * When override_creds=uid:gid is configured, use those values as
	 * fsuid/fsgid so VFS permission checks (including sticky-bit delete
	 * checks) are evaluated against the requested identity.
	 */
	if (ofs->config.has_override_creds) {
		cred->fsuid = ofs->config.override_uid;
		cred->fsgid = ofs->config.override_gid;
	}
