...
static int ovl_check_override_create_identity(struct dentry *dentry)
{
	...
	if (uid_eq(current_fsuid(), ofs->config.override_uid))
		return 0;
				      ...
				      struct list_head *list)
{
	...
	err = ovl_set_attr(ofs, opaquedir, &stat, dentry->d_sb);
			      ...
			      struct ovl_cattr *attr, bool origin)
{
	...
	err = ovl_check_override_create_identity(dentry);
	if (err)
		return err;
	...
	if (!attr->hardlink) {
		 ...
		 */
		/*
		 * On override_creds mounts, keep mounter fs{u,g}id so copy up and
		 * create over lower owned entries do not fail DAC checks.
		 */
		if (!ofs->config.has_override_creds) {
