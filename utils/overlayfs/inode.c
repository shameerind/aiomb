		...
		struct iattr *attr)
{
	...
	/*
	 * With override_creds configured, notify_change() is executed under
	 * creator creds later in this function. Skip the overlay inode ownership
	 * pre-check here to avoid EPERM on metadata preservation operations
	 * (e.g. cp -p chmod/chown/timestamps) for non-owner callers.
	 */
	...
	if (!err) {
		...
		if (err == -EPERM && ofs->config.has_override_creds &&
...
out:
	...
	/*
	 * If override_creds is configured, report all files/dirs as owned
	 * by that uid:gid so that 'ls -l' output is not confusing for the
	 * override user.
	 */
	if (!err) {
		struct ovl_fs *ofs = OVL_FS(dentry->d_sb);
		...
		if (ofs->config.has_override_creds) {
			stat->uid = ofs->config.override_uid;
			stat->gid = ofs->config.override_gid;
		}
	}
		   ...
		   struct inode *inode, int mask)
{
	...
	/*
	 * In override_creds mode, delegate access control to operations that run
	 * with mounter/creator credentials instead of enforcing overlay inode
	 * identity checks in this fast permission path.
	 */
	if (ofs->config.has_override_creds &&
	    uid_eq(current_fsuid(), ofs->config.override_uid))
		return 0;
